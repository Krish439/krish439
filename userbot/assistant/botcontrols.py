...
async def ban_botpms(event):
    ...
    if not user_id:
        return await event.client.send_message(
            event.chat_id, "`मुझे प्रतिबंध लगाने के लिए उपयोगकर्ता नहीं मिल रहा है .`", reply_to=reply_to
        )
    if not reason:
        return await event.client.send_message(
            event.chat_id, "`उपयोगकर्ता को प्रतिबंधित करने के लिए पहले कारण बताएं`", reply_to=reply_to
        )
    try:
...
async def ban_botpms(event):
    ...
    if not user_id:
        return await event.client.send_message(
            event.chat_id, "`मुझे प्रतिबंध हटाने के लिए उपयोगकर्ता नहीं मिल रहा है`", reply_to=reply_to
        )
    try:
        user = await event.client.get_entity(user_id)
        user_id = user.id
...
async def ban_starters(event):
    ...
    if len(ulist) == 0:
        return await eod(event, "`आपके बॉट में अभी तक किसी को भी प्रतिबंधित नहीं किया गया है.`")
