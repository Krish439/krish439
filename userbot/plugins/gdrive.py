...
async def g_download(event):
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, legendevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await eod(
...
async def gshare(event):
    ...
    legendevent = await eor(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, legendevent, input_str)
