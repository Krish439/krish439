...
async def paste_img(event):
    ...
    d_file_name = None
    legendevent = await eor(event, "`Pasting the text on image`")
...
async def paste_bin(event):
    ...
    legendevent = await eor(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await legendevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await eod(legendevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    try:
        ...
        return await legendevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await eor(legendevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await eod(legendevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await legendevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await legendevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
