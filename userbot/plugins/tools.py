...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/LEGEND-AI/LEGENDBOT-Resources/master/Resources/Data/currency.py"
        )
...
async def _(event):
    ...
    chat = "@VS_Robot"
    legendevent = await eor(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await eor(legendevent, response4.text)
...
async def parseqr(event):
    ...
    legendevent = await eor(event, "`Decoding....`")
    ...
    t_response, e_response = (await _legendutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await eor(legendevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await eor(legendevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    legendevent = await eor(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
