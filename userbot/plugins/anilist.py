...
async def anilist_usersearch(event):
    ...
    legendevent = await eor(event, "`Searching user profile in anilist...`")
    searchresult = await anilist_user(search_query)
    if len(searchresult) == 1:
        return await eor(
            legendevent, f"**Error while searching user profile:**\n{searchresult[0]}"
        )
...
@legend.legend_cmd(
    ...
    command=("akaizoku", menu_category),
    info={
...
async def anime_download(event):  # sourcery no-metrics
    ...
    if input_str == "kaizoku":
        ...
        if search_result:
            ...
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>IndiAnime</code>"
    await legendevent.edit(result, parse_mode="html")


@legend.legend_cmd(
    pattern="upcoming$",
    command=("upcoming", menu_category),
    info={
...
@legend.legend_cmd(
    ...
    command=("aschedule", menu_category),
    info={
...
@legend.legend_cmd(
    pattern="w(hat)?anime$",
    command=("whatanime", menu_category),
    info={
