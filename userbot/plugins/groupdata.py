...
@legend.legend_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @OFFICIAL_K_LEGENDBOT",
    },
)
async def _(event):
...
@legend.legend_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @OFFICIAL_K_LEGENDBOT",
    },
)
async def _(event):
    "To get list of bots."
...
@legend.legend_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @OFFICIAL_K_LEGENDBOT",
    },
)
async def info(event):
    "To get group information"
    legendevent = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, legendevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await legendevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await legendevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, legendevent):
    ...
    try:
        ...
        try:
            ...
            await legendevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await eod(legendevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
