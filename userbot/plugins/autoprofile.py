...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await legend.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await legend(
                    functions.photos.DeletePhotosRequest(
                        await legend.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await legend(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await legend(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await legend(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
legend.loop.create_task(autopfp_start())
legend.loop.create_task(autopicloop())
legend.loop.create_task(digitalpicloop())
legend.loop.create_task(bloom_pfploop())
legend.loop.create_task(autoname_loop())
legend.loop.create_task(autobio_loop())
legend.loop.create_task(custompfploop())
