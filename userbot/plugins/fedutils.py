...
async def group_fban(event):
    ...
    if user.id == event.client.uid:
        return await eod(event, "__You can't fban yourself.__")
    if not reason:
        return await eod(
    ...
    if errors != []:
        ...
        for txt in errors:
            success_report += f"\n☞ __{txt}__"
    await eor(legendevent, success_report)


@legend.legend_cmd(
    pattern="unfban(?:\s|$)([\s\S]*)",
    command=("unfban", menu_category),
    info={
...
async def group_unfban(event):
    ...
    if not user:
        return
    if user.id == event.client.uid:
        return await eod(event, "__You can't unfban yourself.__")
    if not reason:
        return await eod(
...
@legend.legend_cmd(
    ...
    command=("addfedto", menu_category),
    info={
...
async def quote_search(event):  # sourcery no-metrics
    ...
    if get_collection("fedids") is not None:
        ...
        feds = {}
    if fedgroup == "-all":
        legendevent = await eor(event, "`Adding all your feds to database...`")
        fedidstoadd = []
...
@legend.legend_cmd(
    ...
    command=("rmfedfrom", menu_category),
    info={
...
async def quote_search(event):
    ...
    if fedid not in fed_ids:
        return await eod(event, "__This fed is not part of given fed category.__")
...
async def myfeds_fedinfo(event):
    ...
    legendevent = await eor(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    legendevent = await eor(event, "`Fetching fedstat from given deatils...`")
    user, fedid = await get_user_from_event(
        event, legendevent, secondgroup=True, noedits=True
    )
    if user is None:
        user = await event.client.get_me()
    if fedid is None:
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            await eod(legendevent, f"**Error while fecthing fedstat:**\n__{e}__", 10)
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
