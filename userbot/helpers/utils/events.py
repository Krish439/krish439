...
async def get_user_from_event(
    event, legendevent=None, secondgroup=None, nogroup=False, noedits=False
):  # sourcery no-metrics
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.from_id is None:
                if not noedits:
                    await eod(legendevent, "`Well that's an anonymous admin !`")
                return None, None
            user_obj = await event.client.get_entity(previous_message.sender_id)
            return user_obj, extra
        if not args:
            if not noedits:
                await eod(legendevent, "`Pass the user's username, id or reply!`", 5)
            return None, None
    except Exception as e:
        LOGS.error(str(e))
    if not noedits:
        await eod(legendevent, "__Couldn't fetch user to proceed further.__")
    return None, None
